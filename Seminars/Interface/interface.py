...
def viewing_contacts():
    ...
    if len(filecontents) == 0: 
        ...
        txt = scrolledtext.ScrolledText(win, width=40, height=10, font = ("Arial", 9))
...
def stop():
    btn_one.config(command=donothing)
    btn_two.config(command=donothing)
    btn_three.config(command=donothing)
    btn_four.config(command=donothing)

def donothing():
    pass

btn_five = Button(win, text="Отмена", font = ("Arial", 9), command=stop)
